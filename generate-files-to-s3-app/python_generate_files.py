...
def generate_random_word(max_words_count_per_line):
    ...
    word_content = " ".join([grab_words[random.randrange(0, len(grab_words))] for i in range(0, max_words_count_per_line)])
    return word_content
...
def produce_file(choose_filename, directory, lines_count, words_per_line):
    ...
    filename = choose_filename + "-" + datetime.datetime.now().strftime("%Y%m%dT%H:%M:%S.%f")
    path = os.getcwd() + '/' + directory
    if not os.path.isdir(path):
        os.makedirs(path)
    with open(directory + "/" + filename, 'a+') as f:
        f.write("Sequence file number : " + "\n")
